...
usage() { echo "$0 usage:     [-t dt] [-e eps] [-x xstart] [-n num] [-m method] [-c MHMC]" && echo "" && echo "Detailed help:" && grep " .)\ #" $0 | sed s/\ \#//g ; exit 1; }

#usage() { echo "$0 usage:" && grep " .)\ #" $0; exit 0; }

while getopts "t:e:x:n:m:c:" arg; do
    case "${arg}" in
...
GSL_RNG_SEED=$(shuf -i 2-999999999 -n 1) ./calc_move.out $DT $EPS $XSTART $NUM $METHOD $MHMC
